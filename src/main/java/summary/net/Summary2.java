...
public class Summary2 {
     ...
     *
     * 项目：
     * 路由路径：根服务器域名的路由路径展示就是某个根在某些省份和运营商下在某段时间里途径节点的有向无环图
     * 还需要算出节点所属IP地址出现的次数
     *
     ...
     * 解析依赖：重要域名依赖的顶级域的一个发散图（例如：www.baidu.com就是由.com.发散到baidu.com发散到ns1.baidu.com发散到www.baidu.com 发散到www.a.shifen.com（这个a.shifen.com就是百度早期的域名））
     * 解析记录：域名对应多条解析结果
     *
     *
     * 漏洞资产
     * 资产（协议规则列表（tcp、udp协议及其端口、备用端口列表）、协议规则统计、资产指纹列表、设备分类列表、事件运营、漏洞事件运营）
     ...
     *
     * http1、1.1与2的区别
     * 1.1 支持长链接，1.1增加了多个错误状态码，半双工
     * 2.0 多路复用，头部压缩（header里信息压缩 避免重复传输）全双工
